...
export async function GET(
  ...
  try {
    ...
    for (const day of days || []) {
      const requiredTasks = day.tasks?.filter((t: { is_required: boolean }) => t.is_required) || [];
      const completedTaskIds = new Set(
        completions?.filter((c) => c.project_day_id === day.id).map((c) => c.task_id) || []
      );
      
      const allRequiredComplete = requiredTasks.every((t: { id: string }) => completedTaskIds.has(t.id));
